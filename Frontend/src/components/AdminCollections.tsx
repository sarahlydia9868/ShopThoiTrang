...
import { getAllCollectionsFromServer } from "../redux/reducers/collections";
import FormInput from "./modules/FormInput";
import Button from "./modules/Button";
import { localStorageData } from "../redux/reducers/user";
...
export default function AdminCollections() {
  ...
  const allCollections = useSelector(
    (state: RootState) => state.collections.allCollections as unknown
  ) as ICollections[];
  ...
  const getProductTitle = (e: any) => {
    setTitle(e.target.value);
  };

  const addNewCollection = () => {
    fetch(`http://localhost:3000/store/collections/`, {
      method: "POST",
      headers: {
  ...
  const confirmModal = () => {
    ...
    fetch(
      `http://localhost:3000/store/collections/${collectionID}/`,
      {
        method: "DELETE",
        headers: {
          Authorization: `JWT ${localStorageData}`,
