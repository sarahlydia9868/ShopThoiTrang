...
export default function NavBarLink({ text, path, dropDownDatas }: INavBarLink) {
  ...
  return (
    <div className="relative group">
      <Link to={path} onClick={scrollToUpHandler}>
        <span
          className={`flex justify-center items-center tracking-wider hover:text-red-500 font-semibold transition-colors duration-300 ease-in-out  ${
            !dropDownDatas &&
            'relative after:absolute after:-right-4 after:w-3 after:h-3 after:bg-[url("/images/svg/star.svg")] after:bg-no-repeat'
          } `}
        >
          {text}
          {dropDownDatas && (
      ...
      {dropDownDatas && (
        <div
          className={`absolute left-0 top-full invisible  group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300 overflow-hidden z-50 pt-7 w-60`}
        >
