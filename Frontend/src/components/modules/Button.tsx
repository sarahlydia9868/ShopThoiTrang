...
export default function Button({ text, padding, bgColor }: IButton) {
  return (
    <div
      className={` overflow-hidden uppercase font-bold ${
        bgColor === "black" ? "bg-black text-white" : bgColor === "white" ? "bg-white" : ""
      } rounded-lg border-1  border-black text-black hover:bg-black hover:text-white  ${padding} cursor-pointer relative after:absolute after:left-0 after:bottom-0 after:bg-[rgba(255,255,255,0.1)] after:border-0  hover:after:border-r-1 after:border-white  z-0 after:-z-10 after:w-0 after:h-full hover:after:w-[101%]  after:transition-all after:ease-in-out after:duration-500 transition-all duration-300 select-none`}
    >
      <span>{text}</span>
    </div>
  );
